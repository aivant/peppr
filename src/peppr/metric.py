...
class BondLengthViolations(Metric):
    ...
    tolerance : float, optional
        The tolerance in Å for acceptable deviation from ideal bond length bounds.
    ...
    def evaluate(self, reference: struc.AtomArray, pose: struc.AtomArray) -> float:
        ...
        invalid_mask = (bond_lengths < lower_bounds - self._tolerance) | (
            bond_lengths > upper_bounds + self._tolerance
        )
...
class BondAngleViolations(Metric):
    ...
    ----------
    tolerance : float, optional
        The tolerance in Å for acceptable deviation from ideal distances.
