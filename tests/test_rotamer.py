...
def test_interp_wrapped_basic(fake_2d_grid):
    """Test the interpolation function with a 2D grid.
    This tests the basic functionality of the interpolation function
    with a simple 2D grid and checks for correct handling of angles.
    """
...
def test_check_rama(
    ...
    The values we compare against are obtained by runnin
    >>> from mmtbx.validation.ramalyze import ramalyze
    >>> import iotbx.pdb
    >>> input_cif = "tests/data/pdb/1a3n.cif"
    >>> pdb_inp = iotbx.pdb.input(file_name=input_cif)
