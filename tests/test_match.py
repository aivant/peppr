...
def test_matching_atoms(pdb_id, crop, seed):
    ...
    model = struc.concatenate(chains)

    reference_order, model_order = peppr.find_matching_atoms(reference, model)
    reordered_reference = reference[reference_order]
    reordered_model = model[model_order]
    reordered_model, _ = struc.superimpose(reordered_reference, reordered_model)

    # Each atom may only appear once
    assert len(np.unique(reference_order)) == len(reference_order)
    assert len(np.unique(model_order)) == len(model_order)
    # Expect all atoms to be contained in the reordered structures
    # (except for the ones that were cropped)
    n_atoms = min(reference.array_length(), model.array_length())
    assert reordered_model.array_length() == n_atoms
    assert reordered_reference.array_length() == n_atoms
    # The model is simply the reference with permuted chains
    # Hence, after reordering the distances should be 0
    assert np.allclose(
        struc.distance(reordered_reference, reordered_model), 0.0, atol=1e-4
    )
...
def test_matching_chains(n_chains, seed, use_different_entities):
    ...
    chain. For the model translations are slightly perturbed.
    Then the chains in the model are randomly swapped and it is expected that
    ...
    # Slightly perturb the reference coordinates to obtain the model
    model_chains = [
    ...
    ref_rmsd = struc.rmsd(
        struc.concatenate(reference_chains), struc.concatenate(model_chains)
    )
    ...
    model = struc.translate(
        struc.concatenate([model_chains[i] for i in swap_indices]),
        rng.normal(scale=SYSTEM_DISTANCE, size=3),
    )
    if use_different_entities:
        # Keep the original entity order
        model.res_name = reference.res_name.copy()
    ...
    _, model_order = peppr.find_matching_atoms(reference, model)
    model = model[model_order]
    superimposed_model, _ = struc.superimpose(reference, model)
    ...
    if use_different_entities:
        ...
        assert model_order.tolist() == list(range(len(model_order)))
    else:
        # Permutation is allowed
        # -> expect that the original minimum RMSD is achieved
        assert struc.rmsd(reference, superimposed_model) <= ref_rmsd
...
def test_shuffled_atom_order(pdb_id, omit_some_atoms, seed):
    ...
    if omit_some_atoms:
        ...
        model = model[
            rng.choice(
                (False, True), p=(P_REMOVED, 1 - P_REMOVED), size=model.array_length()
            )
        ]
    ...
    assert len(model_order) >= min_matching_atoms
    assert len(reference_order) >= min_matching_atoms
    # Ensure that the correct atoms were matched
    assert np.all(reordered_reference.atom_name == reordered_model.atom_name)
    assert np.all(reordered_reference.res_name == reordered_model.res_name)
    assert np.all(reordered_reference.res_id == reordered_model.res_id)
    ...
    assert np.all(np.diff(reordered_model.res_id) >= 0)
...
def test_matching_small_molecules(comp_name, swap_atom_names, shuffle):
    ...
    for atom_name1, atom_name2 in swap_atom_names:
        indices = np.where(np.isin(model.atom_name, (atom_name1, atom_name2)))[0]
        model.coord[indices] = model.coord[indices[::-1]]
    if shuffle:
        # Completely change the order of atoms in the AtomArray
        rng = np.random.default_rng(seed=0)
        model = model[rng.permutation(model.array_length())]

    # To increase difficulty, the atoms are translated -> superimposition is necessary
    model.coord += TRANSLATION_VEC
    ...
    assert model.element.tolist() == reference.element.tolist()
    # After swapping back the model should perfectly overlap with the reference again
    assert struc.rmsd(reference, model) == pytest.approx(0.0, abs=1e-6)
...
def test_no_matching_of_enantiomers(bromochlorofluoromethane, mirror):
    ...
    # For the model move the other coordinates,
    ...
    model = struc.translate(mol_1, TRANSLATION_VEC) + mol_2
    reference_order, model_order = peppr.find_matching_atoms(reference, model)
    if mirror:
        # Mapping enantiomers onto each other is not allowed
        assert model_order.tolist() == reference_order.tolist()
    else:
        ...
        assert (
            np.concatenate(
                [
                    ...
                    model_order[: model_order.shape[0] // 2],
                    model_order[model_order.shape[0] // 2 :],
                ]
            ).tolist()
...
def test_unmatchable_molecules():
    ...
    model.bonds = struc.BondList(model.array_length())
    ...
    assert np.all(model_order == np.arange(model.array_length()))
