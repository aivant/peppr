...
def fake_2d_grid() -> struc.AtomArray:
    ...
    return {'grid': grid, 'axes': [chi1_centers, chi2_centers], 'wrap': wrap, 'span': [(0, 360), (0, 360)]}
...
def test_interp_wrapped_basic(fake_2d_grid):
    ...
    assert np.isclose(val_exact, expected_exact), f"Expected {expected_exact}, got {val_exact}"

    # Case 2: wrapping test: 450° for χ1 wraps to 90°, should give same as (90°, 180°)
    val_wrapped, _ = interp_wrapped(grid_obj, [450.0, 180.0])
    assert np.isclose(val_wrapped, expected_exact), f"Expected {expected_exact} for wrapped input, got {val_wrapped}"
...
def test_get_residue_chis(input_pose, residue_id, chi1, chi2):
    ...
    chis = get_residue_chis(input_pose, (input_pose.res_id == residue_id) & (input_pose.chain_id == "A"))
...
def test_get_residue_phi_psi_omega(input_pose, residue_id, phi, psi, omega):
    ...
    atoms = input_pose[np.isin(input_pose.res_id, [residue_id-1, residue_id, residue_id+1])]
...
@pytest.mark.parametrize(
    [
        "phi", "psi", "omega", "residue_id",
        "resname", "chain_id", "resname_tag",
        "model_no", "rama_score_pct", "classification"
    ],
    [
        # test some edge cases
        # Obtained by running  mmtbx.validation.ramalyze.
...
def test_check_rama(phi, psi, omega, residue_id, resname, chain_id,
                    resname_tag, model_no, rama_score_pct, classification):
    # Test known values
    result = check_rama(
        phi=phi,
        psi=psi,
...
def test_rama_score_from_atom_array(input_pose):
    ...
    assert rama_score.rama_scores[0].resname_tag in [
        "general", "gly", "cispro", "prepro", "transpro", "ileval",
    ]


@pytest.mark.parametrize(
    [
        "chi_angles", "residue_id",
        "resname", "chain_id",
        "model_no", "rotamer_score_pct", "classification"
    ],
    [
        # test some edge cases
        # Obtained by running  mmtbx.validation.ramalyze.
...
def test_check_rotamer(input_pose, chi_angles, residue_id, chain_id, resname, model_no,
                        rotamer_score_pct, classification):
    result = check_rotamer(
        atom_array=input_pose,
        res_id=residue_id,
        chain_id=chain_id,
        model_no=model_no
    )
